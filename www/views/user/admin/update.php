            ...
            <div class="coll-offset-lg-3 coll-lg-6">
                    ...
                    <?php foreach ($data['params'] as $param => $paramList):?>
                        ...
                        <div class="field">
                               ...
                               <input name="<?=$param?>" type="<?=$paramList["html_type"]?>" value="<?=$data['object']->$param?>" id="<?=$param?>">
                        </div>
                            <?php else:?>
                                <div class="field">
                                    <label for="<?=$param?>" class="overrideLabel"><?=$paramList["name"]?></label>
                                    <textarea name="<?=$param?>" id="<?=$param?>" class="textBlock"><?=$data['object']->$param?></textarea>
                                </div>
                        <?php endif;?>
                        <?php endif;?>
                    <?php endforeach;?>
...
<div class="modal-overlay">
    <div class="modal">
        ...
        <div class="modal-content">
            <h3 class="titleModal">Результат оновлення</h3>
            <?php if(isset($data['resultUpdate']) && $data['resultUpdate'] == 1):?>
                <div>
                    <p class="answer">Запис успішно оновлено.</p>
                    <a class="backToTable" href="/admin/show/<?=$data['nameClass']?>">Повернутися до таблиці <?=$data['nameForTable']?></a>
                </div>
            <?php else: ?>
                <div>
                    <p class="answer">При оновленні запису сталася помилка.</p>
                </div>
            <?php endif;?>
        </div>
    </div>
